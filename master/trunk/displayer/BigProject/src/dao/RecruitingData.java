...
public class RecruitingData {
	...
	public List<HighCharts> getRecruitingData(int dist,int type) {
        ...
        try{
			...
			//2、更新数据checkout
			String sql = "select positionName,peopleNum from analysisresult where city=? and type=?";
			...
			while(rs.next()){
				...
				temp.setValue(rs.getDouble("peopleNum"));
		        System.out.println(rs.getString("positionName"));
		        System.out.println(rs.getString("peopleNum"));
		        list.add(temp);
			}
